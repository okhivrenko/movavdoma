...
- `GET /` serves the public Ukrainian landing page; `GET /privacy` serves the
  ...
  clicks. Persisted opt-in is restored with a consent update before GA4 config,
  and CSP permits only Google's documented GA4/GTM collection hosts. The
...
`ig_story`, `tg_ads`, `tg_post`, `tiktok_ads`, `tiktok_story`, or `website`, for example
...
| TikTok Story | `https://movayakvdoma.com/?source=tiktok_story` |
...
the server passes only a validated TikTok campaign value to the bot CTA. The
summary then reports TikTok campaigns such as `tiktok_ads` and `tiktok_story` separately. `direct_or_legacy` combines users who started without an approved
...
- When the 10-word learning-list quota is exhausted, the bot offers a personal
  ...
  quota. Support gives a Monobank link and payment-comment code, followed by
