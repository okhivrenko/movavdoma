...
`ig_bio`, `ig_story`, `tg_ads`, `tg_post`, `tiktok_ads`, `tiktok_story`, and `website`. Unknown
...
TikTok ads and stories use `https://movayakvdoma.com/?source=tiktok_ads` and
`https://movayakvdoma.com/?source=tiktok_story`, not direct bot deep links.
The Worker validates this public query parameter before copying it to a CTA.
To preserve the already-deployed `acquisition_source` database constraint,
TikTok is persisted as source `website` plus a bounded campaign. The admin
report resolves the campaign first and displays it separately.
