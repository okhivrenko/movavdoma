...
`/privacy` збережеться. Сторінка буде server-rendered HTML без сторонніх
скриптів, трекерів або збору персональних даних у першому релізі.

## SEO та вимірювання

- До реалізації: дослідження актуальних українських запитів навколо вивчення
  англійських слів і Telegram-ботів, а також 3–5 коротких перевірок прототипу
  з представниками цільової аудиторії.
- У релізі: унікальні title/description, canonical, Open Graph, семантичні
  headings, релевантний JSON-LD за потреби.
- Після релізу: відокремлювати органічні та кампанійні переходи, перевіряти
  Search Console і Core Web Vitals та формувати backlog з фактичних даних; не
  підключати аналітику без окремого рішення щодо приватності.
